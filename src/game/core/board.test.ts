...
import { DIFFICULTIES, DIFFICULTY_ORDER, presetSpec } from "./constants";
...
describe("createBoard", () => {
  it("starts with no mines at all - mines is null, not an empty array", () => {
    const board = createBoard(presetSpec("beginner"));
...
describe("plantMines - FR-01, checked exhaustively", () => {
  ...
  it.each(DIFFICULTY_ORDER)("never puts a mine in the 3x3 around any opening move (%s)", (
    ...
    const empty = createBoard(presetSpec(difficulty));
  ...
  it("is reproducible from the seed", () => {
    const empty = createBoard(presetSpec("intermediate"));
  ...
  it("leaves the source board untouched", () => {
    const empty = createBoard(presetSpec("beginner"));
